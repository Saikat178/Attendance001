...
-- Check for existing holidays in current year
DO $$
DECLARE
    current_year_holiday_count INTEGER;
    current_year INTEGER;
BEGIN
    current_year := EXTRACT(YEAR FROM CURRENT_DATE);
    
    -- Count holidays in current year
    SELECT COUNT(*) INTO current_year_holiday_count 
    FROM holidays 
    WHERE EXTRACT(YEAR FROM date) = current_year;
    
    -- Only update holidays if needed
    IF current_year_holiday_count = 0 THEN
        -- Update holidays to current year if needed
        UPDATE holidays 
        SET date = (current_year || '-' || EXTRACT(MONTH FROM date) || '-' || EXTRACT(DAY FROM date))::date
        WHERE EXTRACT(YEAR FROM date) != current_year;
        
        RAISE NOTICE 'Updated holidays to current year: %', current_year;
    ELSE
        RAISE NOTICE 'Current year holidays already exist. No updates needed.';
    END IF;
END $$;
