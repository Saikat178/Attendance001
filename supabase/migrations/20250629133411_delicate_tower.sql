...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'employees_phone_format') THEN
        ALTER TABLE employees ADD CONSTRAINT employees_phone_format 
        CHECK (phone IS NULL OR phone ~* '^\+?[1-9]\d{1,14}$');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'employees_salary_positive') THEN
        ALTER TABLE employees ADD CONSTRAINT employees_salary_positive 
        CHECK (salary IS NULL OR salary > 0);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'employees_name_not_empty') THEN
        ALTER TABLE employees ADD CONSTRAINT employees_name_not_empty 
        CHECK (length(trim(name)) > 0);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'employees_employee_id_format') THEN
        ALTER TABLE employees ADD CONSTRAINT employees_employee_id_format 
        CHECK (length(trim(employee_id)) >= 3);
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'employees_verification_documents_status_check') THEN
        ALTER TABLE employees ADD CONSTRAINT employees_verification_documents_status_check 
        CHECK (verification_documents_status IN ('pending', 'verified', 'rejected'));
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'employees_verification_profile_status_check') THEN
        ALTER TABLE employees ADD CONSTRAINT employees_verification_profile_status_check 
        CHECK (verification_profile_status IN ('pending', 'verified', 'rejected'));
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'attendance_valid_hours') THEN
        ALTER TABLE attendance_records ADD CONSTRAINT attendance_valid_hours 
        CHECK (hours_worked >= 0 AND hours_worked <= 24);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'attendance_valid_break_time') THEN
        ALTER TABLE attendance_records ADD CONSTRAINT attendance_valid_break_time 
        CHECK (total_break_time >= 0 AND total_break_time <= 480);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'attendance_checkin_before_checkout') THEN
        ALTER TABLE attendance_records ADD CONSTRAINT attendance_checkin_before_checkout 
        CHECK (check_out IS NULL OR check_in < check_out);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'attendance_break_logic') THEN
        ALTER TABLE attendance_records ADD CONSTRAINT attendance_break_logic CHECK (
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'leave_valid_dates') THEN
        ALTER TABLE leave_requests ADD CONSTRAINT leave_valid_dates 
        CHECK (start_date <= end_date);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'leave_reason_not_empty') THEN
        ALTER TABLE leave_requests ADD CONSTRAINT leave_reason_not_empty 
        CHECK (length(trim(reason)) > 0);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'leave_future_dates') THEN
        ALTER TABLE leave_requests ADD CONSTRAINT leave_future_dates 
        CHECK (start_date >= CURRENT_DATE - INTERVAL '30 days');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'leave_requests_type_check') THEN
        ALTER TABLE leave_requests ADD CONSTRAINT leave_requests_type_check 
        CHECK (type IN ('sick', 'vacation', 'personal', 'emergency'));
    END IF;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'compoff_valid_dates') THEN
        ALTER TABLE comp_off_requests ADD CONSTRAINT compoff_valid_dates 
        CHECK (work_date <= comp_off_date);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'compoff_reason_not_empty') THEN
        ALTER TABLE comp_off_requests ADD CONSTRAINT compoff_reason_not_empty 
        CHECK (length(trim(reason)) > 0);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'compoff_work_date_past') THEN
        ALTER TABLE comp_off_requests ADD CONSTRAINT compoff_work_date_past 
        CHECK (work_date <= CURRENT_DATE);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'comp_off_requests_status_check') THEN
        ALTER TABLE comp_off_requests ADD CONSTRAINT comp_off_requests_status_check 
        CHECK (status IN ('pending', 'approved', 'rejected'));
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'holidays_name_not_empty') THEN
        ALTER TABLE holidays ADD CONSTRAINT holidays_name_not_empty 
        CHECK (length(trim(name)) > 0);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'holidays_valid_date') THEN
        ALTER TABLE holidays ADD CONSTRAINT holidays_valid_date 
        CHECK (date IS NOT NULL);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'holidays_type_check') THEN
        ALTER TABLE holidays ADD CONSTRAINT holidays_type_check 
        CHECK (type IN ('national', 'regional', 'company'));
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'notifications_title_not_empty') THEN
        ALTER TABLE notifications ADD CONSTRAINT notifications_title_not_empty 
        CHECK (length(trim(title)) > 0);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'notifications_message_not_empty') THEN
        ALTER TABLE notifications ADD CONSTRAINT notifications_message_not_empty 
        CHECK (length(trim(message)) > 0);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'notifications_type_check') THEN
        ALTER TABLE notifications ADD CONSTRAINT notifications_type_check 
        CHECK (type IN ('leave_request', 'compoff_request', 'leave_approved', 'leave_rejected', 'compoff_approved', 'compoff_rejected', 'profile_change_request'));
    END IF;
END $$;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'documents_document_type_check') THEN
        ...
        CHECK (document_type IN ('aadhaar', 'pan', 'passport', 'driving_license', 'voter_card', 'education_certificate', 'experience_letter', 'salary_slip'));
    END IF;
END $$;

-- Profile change requests improvements
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'profile_change_requests_status_check') THEN
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_employees_email_lookup ON employees(email);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_employees_employee_id_lookup ON employees(employee_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_employees_role_verified ON employees(role, is_verified);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_attendance_date_range ON attendance_records(date);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_attendance_checkin_time ON attendance_records(check_in) WHERE check_in IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_leave_requests_employee_status ON leave_requests(employee_id, status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_leave_requests_date_range ON leave_requests(start_date, end_date);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_leave_requests_applied_date ON leave_requests(applied_date);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_comp_off_requests_employee_status ON comp_off_requests(employee_id, status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_comp_off_requests_dates ON comp_off_requests(work_date, comp_off_date);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_holidays_date_lookup ON holidays(date);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_holidays_year ON holidays(EXTRACT(YEAR FROM date));
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_holidays_month ON holidays(EXTRACT(MONTH FROM date));
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_holidays_type_optional ON holidays(type, is_optional);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_audit_logs_table_operation ON audit_logs(table_name, operation);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_documents_employee_type ON documents(employee_id, document_type);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profile_change_requests_employee_status ON profile_change_requests(employee_id, status);
...
DROP POLICY IF EXISTS "users_can_manage_own_sessions" ON user_sessions;
...
DROP POLICY IF EXISTS "system_can_manage_rate_limits" ON rate_limits;
...
DROP VIEW IF EXISTS employee_lookup;
CREATE VIEW employee_lookup AS
SELECT 
    id,
    employee_id,
    name,
...
-- Create view for current year holidays
DROP VIEW IF EXISTS current_year_holidays;
CREATE VIEW current_year_holidays AS
SELECT 
    id,
    name,
    date,
    ...
    TO_CHAR(date, 'Day, DD Month YYYY') as formatted_date
FROM holidays
