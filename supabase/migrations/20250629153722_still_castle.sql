/*
  # Fix Data Recording Issues

  1. Database Functions
    - `record_check_in` - Handles employee check-in
    - `record_check_out` - Handles employee check-out  
    - `record_break_start` - Manages break start
    - `record_break_end` - Manages break end
    - `get_today_attendance` - Gets today's attendance record
    - `submit_leave_request` - Handles leave request submission
    - `submit_comp_off_request` - Handles comp-off request submission

  2. Security
    - Ultra-permissive RLS policies for data recording
    - SECURITY DEFINER functions to bypass RLS issues
...
DROP POLICY IF EXISTS "service_role_access" ON attendance_records;
DROP POLICY IF EXISTS "service_role_access" ON leave_requests;
DROP POLICY IF EXISTS "service_role_access" ON comp_off_requests;
DROP POLICY IF EXISTS "service_role_access" ON notifications;
