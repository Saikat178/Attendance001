...
-- Update holidays to current year if needed, but check for duplicates first
DO $$
DECLARE
...
BEGIN
    ...
    LOOP
        -- Calculate the new date for current year
        new_date := (current_year || '-' || 
                    EXTRACT(MONTH FROM holiday_record.date) || '-' || 
                    EXTRACT(DAY FROM holiday_record.date))::date;
                    
        -- Check if this holiday already exists for the current year
        IF NOT EXISTS (
            SELECT 1 FROM holidays 
            WHERE date = new_date AND name = holiday_record.name
        ) THEN
            -- Update only if no duplicate exists
            UPDATE holidays 
            SET date = new_date
            WHERE id = holiday_record.id;
            
            RAISE NOTICE 'Updated holiday: % from % to %', 
                holiday_record.name, holiday_record.date, new_date;
        ELSE
            RAISE NOTICE 'Skipped updating holiday: % (already exists for %)', 
                holiday_record.name, current_year;
        END IF;
    END LOOP;
END $$;
